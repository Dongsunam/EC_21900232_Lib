...
// 1. Initialize GPIO port and pin as AF
		...
		GPIO_ospeed(port, pin, VHIGH);  // speed VHIGH=3
		GPIO_otype(port, pin, 0);
		GPIO_pudr(port, pin, 0);
...
// 3-2. Direction of Counter
		TIMx->CR1 &= ~TIM_CR1_DIR;    // Counting direction: 0 = up-counting
...
void   PWM_duty(PWM_t *pwm, float duty) {                 //  duty=0 to 1	
		float ccval = (pwm ->timer->ARR+1)*duty;    								// (ARR+1)*dutyRatio - 1          
		int CHn = pwm->ch;
		TIM_TypeDef *TIMx = pwm->timer;
